  ...
  void draw(float dt) override {
    ...
    for (uint8_t i = 0; i < STREAMS; i++) {
      ...
      for (uint8_t tail = 0; tail < stream.length; tail++) {
        int16_t y = static_cast<int16_t>(stream.y + tail + 0.5f);
        if (y < 0 || y >= Display::height) continue;

        uint8_t level = stream.brightness / (tail + 1);
        Color color = tail == 0 ? Color(0, 180, 20).scale(level)
                                : Color(0, 130, 10).scale(level);
        voxel(stream.x, y, stream.z, color);
      }
