...
void setup() {
  ...
  config.animation.animation = ANIMATION_ELECTRIC_STORM;
  Animation::begin();
  Serial.println("FlexIO DMA: Electric Storm animation");
}
...
void loop() {
  ...
  if (print_interval.update()) {
    Serial.printf("Electric Storm FPS=%1.2f DMA_ERR=%lx SHIFTERR=%lx\n",
